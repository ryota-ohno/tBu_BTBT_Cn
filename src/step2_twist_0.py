...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','z','A2','phi1','phi2','E','E_p1','E_p2','E_t1','E_t2','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['theta','z','A2','phi1','phi2']
    opt_param_keys = ['a','b']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=4:
            ...
            len_queue-=1
            Et1=float(E_list[0]);Et2=float(E_list[1]);Ep1=float(E_list[2]);Ep2=float(E_list[3])
            E = 2*(Et1+Et2+Ep1+Ep2)
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    num_machine2=len(df_E[(df_E['status']=='InProgress')&(df_E['machine_type']=='2')])
                    if num_machine2 <2:
                        machine_type='2'
                    else:
                        machine_type='1'
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    z = init_params_dict['z']; A2 = init_params_dict['A2']; phi1 = init_params_dict['phi1']; phi2 = init_params_dict['phi2']
        ...
        for a in [a_init_prev]:
            for b in [b_init_prev]:
                ...
                df_val_ab = df_val[
                    ...
                    (df_val['z']==z)&(df_val['A2']==A2)&(df_val['phi1']==phi1)&(df_val['phi2']==phi2)&
                    (df_val['status']=='Done')]
                if len(df_val_ab)==0:
